  ...
  return (
    <div className="bg-surface rounded-xl p-6 lg:p-8 border border-border">
      <div className="mb-6">
        ...
        <p className="text-white/80">Multiple ways to connect and start our conversation.</p>
      </div>
      ...
      <div className="space-y-6">
        {contactDetails.map((contact, index) => (
          <motion.div
            key={contact.label}
            className="flex items-start space-x-4 bg-background/40 rounded-lg p-4 border border-border hover:border-accent/30 nav-transition"
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-medium text-white mb-1">{contact.label}</h3>
              ...
              {contact.href ? (
                <a
                  ...
                  className="text-accent hover:text-accent/80 nav-transition font-medium break-all"
                >
                  {contact.value}
                </a>
              ) : (
                <p className="text-white font-medium break-all">{contact.value}</p>
              )}
