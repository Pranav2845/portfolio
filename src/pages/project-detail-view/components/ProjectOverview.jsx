...
const ProjectOverview = ({ project }) => {
  return (
    <div className="space-y-8">
      {/* Project Overview */}
      <section>
        <h2 className="text-fluid-2xl font-bold text-primary mb-6 flex items-center space-x-3">
          <Icon name="FileText" size={28} className="text-accent" strokeWidth={2} />
          <span>Project Overview</span>
        </h2>
        <div className="prose prose-lg max-w-none text-text-primary">
          <p className="text-fluid-base leading-relaxed">{project.overview}</p>
        </div>
      </section>

      {/* Challenge */}
      <section>
        <h3 className="text-fluid-xl font-bold text-primary mb-4 flex items-center space-x-3">
        ...
        <div className="bg-surface rounded-xl p-6 border-l-4 border-warning">
          <p className="text-fluid-base text-text-primary leading-relaxed">{project.challenge}</p>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="bg-accent/5 rounded-xl p-6 border-l-4 border-accent">
          <p className="text-fluid-base text-text-primary leading-relaxed">{project.solution}</p>
        </div>
      </section>
      ...
      {project.testimonial && (
        <section>
          ...
          <div className="bg-gradient-to-r from-accent/5 to-accent/10 rounded-xl p-8 border border-accent/20">
            <blockquote className="text-fluid-lg text-text-primary italic mb-6 leading-relaxed">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="text-text-secondary text-sm">{project.testimonial.position}</div>
              </div>
            </div>
          </div>
        </section>
