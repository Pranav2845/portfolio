    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
              ...
              {navigationItems.map((item) =>
                item.isDropdown ? (
                  <div key={item.label} ref={aboutRef} className="relative">
                    <button
                      className={`cursor-pointer flex items-center space-x-2 text-fluid-base font-medium nav-transition ${
                        isAboutDropdownOpen
                          ? "text-accent"
                    ...
                    {isAboutDropdownOpen && (
                      <div
                        className="absolute left-0 top-full mt-2 rounded-lg border border-border bg-surface shadow-lg p-2 space-y-1 w-56 z-50"
                        role="menu"
                      >
                        {item.dropdownItems.map((ddItem) => (
                          <Link
                            key={ddItem.path}
                            to={ddItem.path}
                            className={`block px-3 py-2 rounded nav-transition hover:bg-accent/10 ${
            ...
            <button
              ...
              className="md:hidden p-2 rounded-lg text-white/80 hover:text-accent hover:bg-surface nav-transition"
      ...
      {isMobileMenuOpen && (
        ...
        >
          ...
          <div
            className="absolute top-0 left-0 w-80 max-w-[85vw] h-full bg-surface shadow-large mobile-menu-slide open"
          ...
          >
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center justify-between p-6 border-b border-border">
                ...
                <button
                  onClick={closeMobileMenu}
                  className="p-2 rounded-lg text-white/80 hover:text-accent hover:bg-background nav-transition"
              ...
              {/* Mobile Nav */}
              <nav className="flex-1 px-6 py-8" aria-label="Mobile Primary">
                <ul className="space-y-6">
                  {navigationItems.map((item) =>
                    item.isDropdown ? (
                      <li key={item.label} className="text-white/80">
                        <button
                          className="w-full flex items-center justify-between space-x-3 p-3 rounded-lg hover:text-accent hover:bg-background nav-transition"
                          onClick={(e) => {
                            e.preventDefault();
                      ...
                      <li key={item.path}>
                        <Link
                          to={item.path}
                          onClick={closeMobileMenu}
                          className={`flex items-center space-x-3 p-3 rounded-lg hover:text-accent hover:bg-background nav-transition ${
                            isActivePath(item.path) ? "text-accent" : "text-white/80"
                          }`}
                        >
                          <Icon
                            name={item.icon}
